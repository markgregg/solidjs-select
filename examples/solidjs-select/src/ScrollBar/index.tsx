...
const ScollBar: Component<ScollBarProps> = (props: ScollBarProps) => {
  ...
  const initialiseCanvas = () => {
    if (canvasRef) {
      ...
      const length = state().orientation === Vertical ? canvasRef.height : canvasRef.width;
      state().thumbSize =
        Math.floor(
          (length - barLengths) / (state().itemCount - (state().itemsPerPage ?? 1))
        ) < 10
          ? 10
          : Math.floor(
              (length - barLengths) /
                (state().itemCount - (state().itemsPerPage ?? 1))
            );
      state().itemsPerThumb =
        (state().itemCount - (state().itemsPerPage ?? 1)) /
  ...
  createEffect(() => {
    if( props.orientation !== state().orientation ||
      ...
      setTimeout(() => {
        ...
        if( state().item >= state().itemCount ) {
          setTimeout(() => {
            if( canvasRef ) {
              scrollToItem(state().itemCount-1, canvasRef.height, canvasRef.width);
            }
          }, 10);
        }
      }, 10);
  ...
  const scrollToItem = (item: number, h: number, w: number) => {
    state().item = item;
    const length = state().orientation === Vertical ? h : w;
  ...
  const handleMouseDown = (event: MouseEvent) => {
    if (canvasRef) {
      ...
      if (
        ...
        repeat(() => {
          if (state().item + (state().itemsPerPage ?? 1) < state().itemCount) {
            scrollToItem(state().item + 1, ref.height, ref.width);
            return true;
          }
          return false;
  ...
  const isPointInMoveUp = (
    ...
    return (
      (state().orientation === Vertical && y > 0 && y <= arrowLengthAndSpace && x > 0 && x < w) ||
      (state().orientation === Horizontal && x > 0 && x <= arrowLengthAndSpace && y > 0 && y < h)
    );
  };
    ...
    if (canvasRef) {
      ...
      if (state().tracking) {
        ...
        if (
          ...
          !props.hideArrows &&
          isPointInMoveDown(x, y, canvasRef.clientHeight, canvasRef.clientWidth) &&
          state().item < state().itemCount - (state().itemsPerPage ?? 1)
        ) {
          state().hover = 'Down';
          hovering = true;
        }
  ...
  return (
    <canvas
      style={state().orientation === Vertical ? verticalstyle : horizontalstyle}
      ref={canvasRef}
    />
  );
};
